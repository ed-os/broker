...
RUN useradd -d /opt/statusengine -m -s /bin/bash --system naemon && \
    ...
    cd /build && ./autogen.sh --prefix=/opt/statusengine --with-naemon-user=naemon --with-naemon-group=naemon --with-pluginsdir=/usr/lib/nagios/plugins && \
    make -j2 && make install && \
    mkdir -p /opt/statusengine/var/ /opt/statusengine/var/cache/naemon /opt/statusengine/var/spool/checkresults /opt/statusengine/etc/naemon/module-conf.d && \
    chown naemon:naemon /opt/statusengine/var /opt/statusengine/etc  -R && \
    cd / && rm -r /build && rm /v${NAEMON_VERSION}.tar.gz

COPY devtools/buildenv/statusengine.toml /opt/statusengine/etc/
COPY devtools/buildenv/statusengine.cfg /opt/statusengine/etc/naemon/module-conf.d/
...
CMD ["/usr/bin/wait-for-it.sh", "rabbit:5672", "--", "/opt/statusengine/bin/naemon", "/opt/statusengine/etc/naemon/naemon.cfg"]

COPY . /broker/source/

RUN cd /broker/build && export PKG_CONFIG_PATH=/opt/statusengine/lib/pkgconfig && cmake -DCMAKE_INSTALL_PREFIX:PATH=/opt/statusengine ../source && make -j2 && make install
